...
describe('scenic places provider', () => {
  it('maps named OpenStreetMap elements into explore places', async () => {
    const fetchImpl = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ elements: [
      { type: 'node', id: 42, lat: 12.9, lon: 77.6, tags: { name: 'Hill View', tourism: 'viewpoint', image: 'https://images.example/hill.jpg' } },
    ...
    expect(places[0]).toMatchObject({ id: 'osm-node-42', name: 'Hill View', category: 'Viewpoints', source: 'openstreetmap', imageRef: 'https://images.example/hill.jpg' });
